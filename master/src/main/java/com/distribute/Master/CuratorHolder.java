...
public class CuratorHolder {
	...
	public static CuratorHolder instance;
	...
	public CuratorHolder() {
		instance = this;
		this.setUpConnection(MasterManager.ZK_HOST);
	}
	...
	public String createNode(String registerPath, String value, CreateMode nodeType) throws Exception {
		...
		if (nodeType == null) {
			...
			return client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).forPath(registerPath,
					value.getBytes());
		} else if (CreateMode.EPHEMERAL.equals(nodeType)) {
			return client.create().creatingParentsIfNeeded().withMode(CreateMode.EPHEMERAL).forPath(registerPath,
					value.getBytes());
			...
			return client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT_SEQUENTIAL)
					.forPath(registerPath, value.getBytes());
	 ...
	 * @throws Exception
	 */
	public void listenChildNodes(String targetPath, PathChildrenCacheListener listener) throws Exception {
		PathChildrenCache childrenCache = new PathChildrenCache(client, targetPath, true);
